  ...
  container.innerHTML = `
    <main class="max-w-6xl mx-auto px-4 py-8 pb-32">
      ...
      <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
        <button onclick="filterOrderMenu('all', this)" class="cat-btn ${orderCurrentCategory === "all" ? "active-cat" : ""}">Semua</button>
        <button onclick="filterOrderMenu('Food', this)" class="cat-btn ${orderCurrentCategory === "Food" ? "active-cat" : ""}">Food</button>
        <button onclick="filterOrderMenu('Snack', this)" class="cat-btn ${orderCurrentCategory === "Snack" ? "active-cat" : ""}">Snack</button>
        <button onclick="filterOrderMenu('Beverage', this)" class="cat-btn ${orderCurrentCategory === "Beverage" ? "active-cat" : ""}">Beverage</button>
      </div>

      <div id="orderMenuGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div>
    </main>
    ...
    <div id="orderCartSummary" class="cart-summary">
      <div class="max-w-6xl mx-auto flex items-center justify-between gap-4">
        ...
        <button onclick="proceedToOrderCheckout()" class="btn btn-terra">Checkout →</button>
      </div>
    </div>
  `;
...
/**
 * ✅ DETAIL ORDER - VERSI BERSIH (tanpa verifikasi manual)
 */
function openOrderDetail(orderId) {
  ...
  // ✅ 1. Payment Proof Section (LANGSUNG TERVERIFIKASI - tanpa tombol verifikasi)
  const paymentProofHTML = order.paymentProof
    ? `
  ...
  // ✅ 3. Reservation Info (FIXED: && bukan & &)
...
// ✅ HAPUS fungsi verifyPayment() karena tidak diperlukan lagi
  ...
  container.innerHTML = `
    <main class="max-w-2xl mx-auto px-4 py-8">
      ...
      <form id="deliveryForm" class="space-y-6">
        <div class="card">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input type="text" id="deliveryName" required value="${existingData.name || ""}" placeholder="Masukkan nama penerima" class="input" />
            </div>
            <div>
              <label class="input-label">Nomor WhatsApp *</label>
              <input type="tel" id="deliveryPhone" required value="${existingData.phone || ""}" placeholder="08xxxxxxxxxx" class="input" />
        ...
        <div class="card">
          ...
          <div class="space-y-4">
            <div>
              ...
              <textarea id="deliveryAddress" required rows="4" placeholder="Jalan, nomor rumah, RT/RW, kelurahan, kecamatan, kota, kode pos" class="input resize-none">${existingData.address || ""}</textarea>
            ...
            <div>
              ...
              <input type="text" id="deliveryNotes" value="${existingData.notes || ""}" placeholder="Contoh: Patokan warna pagar, lantai 2, dll" class="input" />
            </div>
          </div>
        </div>
        ...
        <button type="submit" class="btn btn-terra btn-full">Lanjut ke Pembayaran →</button>
      </form>
    </main>
  `;
...
function submitDeliveryData() {
  ...
  const deliveryData = { name, phone, address, notes };
  ...
  Modal.confirm({
    ...
    html: `
      <div class="space-y-3">
        ...
        <p class="text-xs text-gray-500 text-center">Pembayaran wajib di awal (QRIS/Transfer). Tidak ada COD.</p>
      </div>
    `,
