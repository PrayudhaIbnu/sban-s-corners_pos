...
function getFilteredReservations() {
  ...
  switch (currentReservationFilter) {
    ...
    case 'upcoming':
      return reservations.filter(r => r.date >= today && r.status !== 'cancelled' && r.status !== 'checked-out');
    case 'all':
      return reservations;
    default:
      return reservations.filter(r => r.date === today);
...
function updateReservationStats() {
  ...
  const checkedIn = todayReservations.filter(r => r.status === 'checked-in').length;
...
function checkInReservation(reservationId) {
  ...
  Modal.confirm({
    ...
    size: 'md',
    onConfirm: () => {
      StorageBridge.updateReservationStatus(reservationId, 'checked-in', 'Pelanggan check-in oleh kasir');
      Modal.success({
...
function checkOutReservation(reservationId) {
  ...
  Modal.confirm({
    ...
    size: 'md',
    onConfirm: () => {
      StorageBridge.updateReservationStatus(reservationId, 'checked-out', 'Pelanggan check-out');
      Modal.success({
      ...
      <div class="space-y-4">
        ...
        ${reservation.statusHistory && reservation.statusHistory.length > 0 ? `
          <div>
            ...
            <div class="space-y-2">
              ${reservation.statusHistory.slice().reverse().map(h => `
...
function showReservationNotification(reservation) {
  ...
  toast.className = 'fixed top-20 right-4 bg-white border-2 border-forest rounded-2xl shadow-2xl p-4 z-50 max-w-sm animate-slide-in-right';
  toast.innerHTML = `
...
function validateBarcode() {
  ...
  if (!reservation) {
    Modal.error({
      ...
      message: 'Reservasi dengan barcode tersebut tidak ditemukan dalam sistem.',
      icon: 'scan-x',
